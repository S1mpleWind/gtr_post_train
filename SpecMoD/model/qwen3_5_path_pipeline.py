...
class Qwen3_5TextModel(Qwen3_5PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        all_hidden_states += (hidden_states,) #the last layer
            # [jyf]
                ...
                for layer in range(1, self.config.num_hidden_layers+1):
                    ...
                    if layer-1 in forced_list:
                        for budget in range(x+1):
                            layer_mask = linear_attn_mask if self.layers[layer-1].layer_type == "linear_attention" else causal_mask
                        ...
                        for budget in range(1, x):
                            ...
                            # [bs, seq, dim]
                            cos_sim_exec = torch.nn.functional.cosine_similarity(layer_outputs, all_hidden_states[layer], dim=-1).mean()
                            ...
                            if cos_sim_exec >= cos_sim_skip:
                                dp[x][budget] = layer_outputs #*[0] return changed
                    ...
                    if token_id_pred == token_id_truth_fake:
                        ...
                        for i in  range(self.config.num_hidden_layers):
                            if i not in path[end][budget]:
                                ...
                                #if cos_sim_veri < 0.88: # tODO try
                                if cos_sim_veri < 0.92:
                                    flag = False
                                    break
                        if flag:
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if input_ids.shape[-1] > 1: #prefill
            ...
            #past_key_values = outputs.past_key_values #TODO: of what use?
        else:
            past_key_values_back = copy.deepcopy(past_key_values)
            exec_layer_list = []
            outputs: BaseModelOutputWithPast = self.model(
        ...
        if input_ids.shape[-1] ==1: # decode
            ...
            json_item = {"layer_index": exec_layer_list[0], 
                         "input_id": input_ids[0,-1].item(), 
                         "output_id": token_id_pred.item()}
