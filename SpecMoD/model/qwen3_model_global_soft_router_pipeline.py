...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # TODO always calculate hidden state instead
        # if not flag:
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if gate_scores_override is not None:
            ...
            #*改成跟 inputs_embeds 同 dtype
            gate_scores = (logits > 0.0).to(dtype=inputs_embeds.dtype)

            # 统一成 [B, L, 1]，其中 L = num_hidden_layers
            if gate_scores.dim() == 2:
                # [B, L] -> [B, L, 1]
                gate_scores = gate_scores.unsqueeze(-1)
            elif gate_scores.dim() == 3:
                # 可能是 [B, 1, L]，转成 [B, L, 1]
                if gate_scores.shape[1] == 1 and gate_scores.shape[2] == self.config.num_hidden_layers:
                    gate_scores = gate_scores.transpose(1, 2)
                # 可能已经是 [B, L, 1]
                elif gate_scores.shape[1] == self.config.num_hidden_layers and gate_scores.shape[2] == 1:
                    pass
                else:
                    raise ValueError(f"Unexpected gate_scores shape: {gate_scores.shape}")
            else:
                raise ValueError(f"Unexpected gate_scores dim: {gate_scores.dim()}")
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        outputs: BaseModelOutputWithPast = self.model(
            ...
            gate_scores_override=gate_scores_override, 
            **kwargs,
        )
