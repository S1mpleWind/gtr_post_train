...
def main(args):
    from model.qwen3_model_eval_speedup_block_router import  Spec_Qwen3ForCausalLM
    ...
    for i in range(0, 32//args.block_size):
        block_router = PathPredictorMLP(n_layers=args.block_size, llm_hidden_dim=model.config.hidden_size, mlp_internal_dim=1024)
        block_router_weight = torch.load(f"./checkpoint/block_router/1024/router_block_{i}_1024.pt")
        block_router.load_state_dict(block_router_weight)
        block_router = block_router.half().to(model.device)
        router.append(block_router)
    ...
    for question in tqdm.tqdm(questions):
        ...
        if args.optimize:
            for i in range(10):
                start = time.time()
                outputs = model.generate(**inputs, max_new_tokens=args.max_gen, temperature=0.000001, adaptor=adaptor, router=router, block_size = args.block_size)
                end = time.time()
        else:
            for i in range(10):
                start = time.time()
