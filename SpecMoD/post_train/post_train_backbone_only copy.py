...
def _collect_decode_logits_for_positions_batch(
    ...
    outputs = model(
        ...
        spec_model=spec_model,
        last_hidden_state=None,
        output_hidden_states= config["train_hidden"], # TODO hidden
    ...
    if config["train_hidden"]:
        hidden = outputs.hidden_states
        return out_map_batch, hidden
...
def train_func(config):
    ...
    ori_model = Spec_Qwen3ForCausalLM.from_pretrained(model_path).bfloat16().to(device)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            #TODO hidden
            result = _collect_decode_logits_for_positions_batch(
                model=backbone, 
                spec_model=spec_model,
                router=router,
            ...
            if config["train_hidden"]:
                logits_map_batch, hidden_batch = result
            else:
                logits_map_batch = result
                hidden_batch = None
            ...
            for b in range(B):
                ...
                for p, tgt_id, topk_ids, topk_logits in zip(
                    ...
                    # TODO hidden SCE
                    if config["train_hidden"]:
                        # teacher_hidden: [hidden_dim]
                        teacher_hidden = torch.tensor(batch["token_hiddens"][b][i], device=device, dtype=torch.float32)
                        # student_hidden: [hidden_dim]，从 hidden_batch 取出
                        student_hidden = hidden_batch[b, p-1, :].float()  # p-1 因为hidden和logits对齐
                        hidden_sce = F.mse_loss(student_hidden, teacher_hidden, reduction="mean")
                        all_hidden_sce_losses.append(hidden_sce)
            ...
            # TODO
            local_no_loss = (len(all_ce_losses) == 0 and len(all_kd_losses) == 0)
            ...
            if len(all_hidden_sce_losses) > 0:
                hidden_sce_loss = torch.stack(all_hidden_sce_losses).mean()
                loss_parts.append(config.get("hidden_sce_weight", 1.0) * hidden_sce_loss)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str,
                        default="/home/xujiaming/xujiaming/jiaoyifan/gtr_post_train/SpecMoD/checkpoint/backbone_only")
    ...
    # argparse
    parser.add_argument("--train_hidden", action="store_true", help="是否启用hidden SCE损失")

    args = parser.parse_args()

    tokenizer = AutoTokenizer.from_pretrained("/share/public/public_models/Qwen3-8B")
