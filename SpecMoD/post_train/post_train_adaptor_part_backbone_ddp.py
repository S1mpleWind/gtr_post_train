...
def _collect_decode_logits_for_positions_batch(
    ...
    # DDP处理: 暴露被包装的原模块以便可以直接调用它的属性如 model.config, model.model 等
    raw_model = model.module if isinstance(model, DDP) else model
    raw_adaptors = [ad.module if isinstance(ad, DDP) else ad for ad in adaptors]

    B, L = input_ids_2d.shape
    out_map_batch = [{} for _ in range(B)]
    raw_model.model.input_id_init()
    spec_model.reset_kv()
    with torch.no_grad():
        prefill_outputs = model(  # 使用具有DDP的前向
            input_ids=input_ids_2d,
            attention_mask=attention_mask_2d,
            use_cache=False,
            adaptor=raw_adaptors,
            router=router,
    ...
    if input_ids_2d.size(1) <= 1:
        gate_scores = torch.ones(
            (B, raw_model.config.num_hidden_layers, 1),
        ...
        emb_for_router = raw_model.get_input_embeddings()(input_ids_2d[:, -1:])
    ...
    raw_model.model.input_id_init()
    spec_model.reset_kv()
    outputs = model( # 保证此处也能够利用DDP广播梯度
        input_ids=input_ids_2d,
        attention_mask=attention_mask_2d,
        use_cache=False,
        adaptor=raw_adaptors,
...
def train_adaptors_end_to_end(args):
    ...
    if is_main_process:
        ...
        os.makedirs(args.save_backbone_dir, exist_ok=True)
    ...
    # 2. 解冻：只解冻最后 4 层（不包含 lm_head 和 norm）
    ...
    for name, param in ori_model.named_parameters():
        ...
        if any([f"layers.{j}." in name for j in range(num_layers - n_tune, num_layers)]):
            if "norm" not in name: 
    ...
    # 将包含可训练参数的 ori_model 套上 DDP
    ori_model = DDP(ori_model, device_ids=[local_rank], find_unused_parameters=True)
    ...
    # raw_ori_model 用于获取 config 等不支持 DDP 直传的部分属性
    raw_ori_model = ori_model.module

    LAYERS = raw_ori_model.config.num_hidden_layers
    router = Global_router(
        input_dim=raw_ori_model.config.hidden_size * 2,
        hidden_dim=1024,
        output_dim=LAYERS,
    ).to(device)
    ...
    for i in range(1, LAYERS):
        if i == 34:
            ...
            layer_adaptor = ShadowAdapter3(raw_ori_model.config.hidden_size, args.adaptor_hidden_dim)
            ...
            # 使用 DDP 包装并保存
            layer_adaptor = DDP(layer_adaptor, device_ids=[local_rank], find_unused_parameters=False)
            adaptors.append(layer_adaptor)
            all_params += list(layer_adaptor.parameters())
            
    all_params += [p for p in ori_model.parameters() if p.requires_grad]

    adaptor_params = []
    for i, adaptor in enumerate(adaptors):
        if adaptor is not None:
            adaptor_params += list(adaptor.parameters())
    backbone_params = [p for p in ori_model.parameters() if p.requires_grad]
    ...
    train_loader = DataLoader(
        ...
        sampler=sampler, # 使用了sampler，不要用shuffle=True
        num_workers=0,
    ...
    for epoch in range(args.num_epochs):
        sampler.set_epoch(epoch) # 保证每个 epoch 数据切分及打乱不一致
        total_loss = 0.0
        total_ce = 0.0
        total_kd = 0.0
        step_count = 0
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            for b in range(B):
                ...
                for p, tgt_id, topk_ids, topk_logits in zip(pos_list, tgt_list, tk_ids_list, tk_logits_list):
                    ...
                    if not torch.isfinite(s).all():
                        if is_main_process: print("infinite s")
                        continue
            ...
            if is_main_process and args.debug_grad_stats and (global_step % args.debug_grad_interval == 0):
                print(
            ...
            if grad_pre > 1000 or not torch.isfinite(torch.tensor(grad_pre)):
                if args.printWarn and is_main_process:
            ...
            with torch.no_grad():
                for i, adaptor in enumerate(adaptors):
                    if adaptor is None:
                        continue
                    # 处理DDP下的参数边界
                    for name, param in adaptor.module.named_parameters():
            ...
            if (epoch + 1) % args.save_interval == 0:
                for i, adaptor in enumerate(adaptors):
                    if adaptor is not None:
                        ...
                        torch.save(adaptor.module.state_dict(), save_path)
                print(f"✓ 保存到: {args.save_dir}")
                backbone_save_path = os.path.join(args.save_backbone_dir, f"backbone_final.pt")
                torch.save(ori_model.module.state_dict(), backbone_save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kd_temperature", type=float, default=2)
    ...
    parser.add_argument("--log_dir", type=str, default="./runs/adaptor_backbone_post_train")
    parser.add_argument("--log_interval", type=int, default=8)
    parser.add_argument("--save_dir", type=str, default="/home/xujiaming/xujiaming/jiaoyifan/gtr_post_train/SpecMoD/checkpoint/adaptor_with_backbone")
