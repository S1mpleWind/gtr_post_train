...
def train_adaptors_end_to_end(args):
    ...
    # 仅主进程创建目录和 TensorBoard
    if global_rank == 0:
        os.makedirs(args.save_dir, exist_ok=True)
    
    writer = None
    if args.use_tensorboard and global_rank == 0:
    ...
    for i in range(1, LAYERS):
        if i == 34:
            ...
            # 2. 对每个 Adaptor 模块进行 DDP 包装
            layer_adaptor = DDP(layer_adaptor, device_ids=[local_rank], output_device=local_rank)
    ...
    # 3. 使用 DistributedSampler 对数据进行切片
    ...
    for epoch in range(args.num_epochs):
        # 让 DistributedSampler 在每个 epoch 以不同的种子打乱数据
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            for b in range(B):
                ...
                for p, tgt_id, topk_ids, topk_logits in zip(pos_list, tgt_list, tk_ids_list, tk_logits_list):
                    ...
                    if args.debug_compare and global_rank == 0: # 仅在 rank0 打印对比信息
            ...
            # 多卡环境下，backward 会自动执行梯度的 All-Reduce
            batch_loss.backward()
            ...
            if args.debug_grad_stats and (global_step % args.debug_grad_interval == 0) and global_rank == 0:
                print(
            ...
            if grad_pre > 800 or not torch.isfinite(torch.tensor(grad_pre)):
                if global_rank == 0 and args.printWarn:
            ...
            if bad_grad and args.printWarn:
                if global_rank == 0:
        ...
        if global_rank == 0:
            ...
            # 4. 仅主卡进行模型保存，使用 .module.state_dict() 取出原始参数
...
if __name__ == "__main__":
    ...
    # DDP 中的 batch_size 为单卡 batch_size
    ...
    parser.add_argument("--max_train_samples", type=int, default=2000, help="小数据集试跑样本数")
    ...
    parser.add_argument("--log_interval", type=int, default=8, help="每多少 step 记录一次")
    ...
    parser.add_argument("--max_distill_tokens_per_sample", type=int, default=128, help="每个样本最多蒸馏多少个 token")
    parser.add_argument("--debug_grad_stats", type=bool, default=False, help="打印梯度和 token 统计")
    parser.add_argument("--debug_grad_interval", type=int, default=1, help="每多少 step 打印一次梯度统计")
